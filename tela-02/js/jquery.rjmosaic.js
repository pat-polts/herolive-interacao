/**
 * Ricardo e Jose Mosaic
 *
 * Pinterest like responsive image grid that doesn’t sucks
 *
 * @requires jQuery
 * @author Artem Sapegin
 * @copyright 2016 Artem Sapegin, http://sapegin.me
 * @license MIT
 */
...
(function(factory) {  // Try to register as an anonymous AMD module
	...
	$.fn.rjmosaic = function(options) {
		var args = Array.prototype.slice.call(arguments,0);

		return this.each(function() {
			var elm = $(this);
			var data = elm.data('rjmosaic');

			if (!data) {
				options = $.extend({}, $.fn.rjmosaic.defaults, options, dataToOptions(elm));
				data = new rjmosaic(elm,options);
				elm.data('rjmosaic', data);
			}
	...
	$.fn.rjmosaic.defaults = {
		itemSelector: '> *',
		columnClass: 'rjmosaic__column',
		minItemWidth: 40,
		minColumns: 2,
	...
	function rjmosaic(container, options) {
		this.container = container;
		this.options = options;

		this.container.trigger('rjmosaic-start');
		this.init();
		this.container.trigger('rjmosaic-ready');
	}
	...
	rjmosaic.prototype = {
		...
		refill: function() {
			this.container.trigger('rjmosaic-fill');
			...
			this.container.trigger('rjmosaic-filled');
		},
		...
		fillColumns: function() {
			...
			this.container.trigger('rjmosaic-layout');
		},
		...
		add: function(elm) {
			this.container.trigger('rjmosaic-item-add', [elm]);
			...
			this.levelBottomEdge(this.itemsHeights, this.columnsHeights);
			this.container.trigger('rjmosaic-layout');
			this.container.trigger('rjmosaic-item-added', [elm]);
		},

		remove: function(elm) {
			this.container.trigger('rjmosaic-item-remove', [elm]);
			...
			this.levelBottomEdge(this.itemsHeights, this.columnsHeights);
			this.container.trigger('rjmosaic-layout');
			this.container.trigger('rjmosaic-item-removed', [elm]);
		},
		...
		generateUniqueId: function() {
			...
			// Return an unique ID
			return 'rj-' + this.__uid + '-itemid-' + this.__uidItemCounter;
		}
	};
	...
	// Auto init
	$(window).load(function() { $('.rjmosaic').rjmosaic(); });
